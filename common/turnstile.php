...
function huli_turnstile_widget_html()
{
    ...
    if (!$script_printed) {
        $html .= '<script>'
            ...
            . 'if(!window.huliTurnstileRetryTimer){'
            . 'var attempts=0;'
            . 'window.huliTurnstileRetryTimer=setInterval(function(){'
            . 'attempts++;'
            . 'if(window.turnstile){window.huliTryRenderTurnstiles();clearInterval(window.huliTurnstileRetryTimer);window.huliTurnstileRetryTimer=null;}'
            . 'else if(attempts>=40){clearInterval(window.huliTurnstileRetryTimer);window.huliTurnstileRetryTimer=null;}'
            . '},250);'
            . '}'
            . '};'
            . '</script>'
