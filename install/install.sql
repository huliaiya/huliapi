...
CREATE TABLE `huli_orders` (
  ...
  `provider` varchar(20) NOT NULL DEFAULT 'afdian' COMMENT '支付提供方',
  `provider_order_id` varchar(64) DEFAULT NULL COMMENT '爱发电订单号',
  `provider_user_id` varchar(64) DEFAULT NULL COMMENT '爱发电用户ID',
  `provider_amount` decimal(10,2) DEFAULT NULL COMMENT '爱发电实付金额',
  `provider_status` varchar(20) DEFAULT NULL COMMENT '爱发电原始状态',
  `provider_paid_at` timestamp NULL DEFAULT NULL COMMENT '爱发电支付时间',
  `match_code` varchar(32) DEFAULT NULL COMMENT '爱发电备注码',
  `query_attempts` int(11) NOT NULL DEFAULT 0 COMMENT '查询次数',
  `last_queried_at` timestamp NULL DEFAULT NULL COMMENT '最近查询时间',
  `fulfilled_at` timestamp NULL DEFAULT NULL COMMENT '权益发放时间',
...
CREATE TABLE `huli_email_broadcasts` (
  ...
  `send_type` enum('once','daily') NOT NULL DEFAULT 'once' COMMENT '发送模式：once=仅一次 daily=每日定时',
  `scheduled_at` datetime DEFAULT NULL COMMENT '计划发送时间/每日发送时刻',
  `last_run_at` datetime DEFAULT NULL COMMENT '最近一次实际发送时间',
  `last_error` varchar(500) DEFAULT NULL COMMENT '最近一次发送错误信息',
...
DROP TABLE IF EXISTS `huli_update_jobs`;
CREATE TABLE `huli_update_jobs` (
...
DROP TABLE IF EXISTS `huli_update_history`;
CREATE TABLE `huli_update_history` (
  ...
  KEY `idx_token` (`token`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='在线更新历史（重要数据落库）';
