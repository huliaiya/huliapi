...
$('#iframe-content').multitabs({
    ...
    init: [{
        type: 'main',
        title: '首页',
        url: $('#default-page').attr('href') + '?_=' + new Date().getTime(),
        active: true
    }]
});

// 为所有multitabs链接动态添加时间戳
$(document).on('click', '.multitabs', function(e) {
    var $this = $(this);
    var originalHref = $this.attr('href');
    
    // 只处理没有时间戳的链接
    if(originalHref && originalHref.indexOf('?_=') === -1) {
        $this.attr('href', originalHref + '?_=' + new Date().getTime());
    }
});

// 确保所有multitabs链接都带时间戳
$(document).on('click', '.multitabs', function() {
    var url = $(this).attr('href');
    if(url.indexOf('?') === -1) {
        $(this).attr('href', url + '?_=' + new Date().getTime());
    }
});
